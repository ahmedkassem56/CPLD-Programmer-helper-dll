...
int DLL_EXPORT startProgramming(int compare, int verbose, char* filename, char* logname, char* errname)
{
	FT_HANDLE ftHandle;
	FT_STATUS ftStatus;
	FILE *fp;
    ...
    freopen (logname,"w",stdout); // redirect stdout to log file
    setbuf(stdout, NULL);

    freopen (errname,"w",stderr); // redirect stderr to log file
    setbuf(stderr, NULL);
